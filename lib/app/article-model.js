...
module.exports = function createArticleModel(connection) {
  ...
  Schema.statics = {
    ...
    list: function(userId, n) {
      return new Promise(function(resolve, reject) {
        if (Date.now() - listCacheUpdated < 5000) {
          return resolve(listCache.map(toUser));
        }
        ...
        function onArticles(err, articles) {
          if (err) return reject(err);
          listCache = articles.sort(byTime).sort(byScore);
          listCacheUpdated = Date.now();
          resolve(listCache.map(toUser));
        }
      }.bind(this));
