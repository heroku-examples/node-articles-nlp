...
module.exports = function articlesRouter(app) {
  ...
  function upVoteArticle(req, res, next) {
    ...
    function sendVotes(article) {
      // TODO: figure out exactly how this hapepns
      if (!article || !article.voteCount) return next(new ArticleNotFound());
      if (article) return res.json({ votes: article.voteCount });
      next(new ArticleNotFound());
    }
  }
};
